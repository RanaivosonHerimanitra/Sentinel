...
if ( input$Cluster_algo=="Total")
 {
  ...
  cat('calculating number of intervention in mild by code (date) and by facies...')
   Nbsites_received=caid[caid_value==1 & get(input$Cluster_algo)==1,length(unique(sites)),by="code"]
   setnames(Nbsites_received,"V1","numerateur")
   Nbsites_actif=caid[get(input$Cluster_algo)==1,length(unique(sites)),by="code"]
   setnames(Nbsites_actif,"V1","denominateur")
   caid= merge(Nbsites_received,Nbsites_actif,by.x=c("code"),by.y=c("code"),all.x=T)
   caid[,caid_value:=100*numerateur/denominateur]
  cat('DONE\n')
