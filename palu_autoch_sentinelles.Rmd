...
lastweek =  unique(palu_autoch[as.Date(deb_sem)==max_deb_sem-7, list(sites,autoch)])
lastweek[,autoch:=as.character(autoch)]
lastweek[is.na(autoch)==T,autoch:="na"]
last6_palu_autoch= merge(unique(last6_palu_autoch[,list(sites,vals)]),lastweek, by.x="sites", by.y="sites")
             
# merge with sentinel_code :
sentinel_code=fread('data/sentinel_codes.csv')
sentinel_code[,sites:=tolower(sites)]
last6_palu_autoch=merge(last6_palu_autoch,
                        sentinel_code,by.x="sites",by.y="sites")
#
setnames(last6_palu_autoch,c("name","vals","autoch"),
                c("Sites","6 semaines précédentes","Semaine dernière"))

# #generate tables:
knitr::kable(last6_palu_autoch[, c("Sites","6 semaines précédentes","Semaine dernière"),with=F],
