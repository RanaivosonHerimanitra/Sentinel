...
```{r,echo=F,out.width = '100%', out.extra='',comment='', message=FALSE , warning=FALSE}
#load source code:
source("import1.r");source("preprocessing.R");require(ggplot2); require(gridExtra)
mydata= preprocessing_disease()[["Diarrhée"]] #Diarrhée
...
Diarrh_feb$malaria_cases=as.character(Diarrh_feb$diarh_feb)
Diarrh_feb[,malaria_cases:=ifelse(is.na(malaria_cases)==T,"XX",diarh_feb)]
...
Diarrh_feb[,malaria_cases:=NULL]
Diarrh_feb= merge(unique(Diarrh_feb[,list(sites,vals)]), 
               unique(mydata[as.Date(deb_sem)==max_deb_sem-7,list(sites,diarh_feb)]),
               by.x="sites", by.y="sites")
Diarrh_feb= merge(Diarrh_feb,sentinel_latlong[,list(name,sites)],
                  by.x="sites",by.y="sites")
Diarrh_feb[,sites:=NULL]; setnames(Diarrh_feb,c("name","vals","diarh_feb"),
...
Diarrh[,diarh:=ifelse(is.na(diarh)==T,"XX",diarh)]
Diarrh[,diarh:=ifelse(nchar(diarh)<2,paste0("0",diarh),diarh)]
#inline historical occurence:
Diarrh[,vals:=paste(diarh,collapse = "-"),by="sites"]
#append latest week per site:
Diarrh[,diarh:=NULL]
Diarrh= merge(unique(Diarrh[,list(sites,vals)]), 
               unique(mydata[as.Date(deb_sem)==max_deb_sem-7,list(sites,diarh)]),
               by.x="sites", by.y="sites")
#merge with sentinel_latlong:
Diarrh= merge(Diarrh,sentinel_latlong[,list(name,sites)],
              by.x="sites",by.y="sites")
Diarrh[,sites:=NULL]; setnames(Diarrh,c("name","vals","diarh"),
                               c("Sites","6 semaines précédentes","Semaine dernière"))
...
knitr::kable(Diarrh_feb[,c("Sites","6 semaines précédentes","Semaine dernière"),with=F],
             caption="Diarrhées fébriles dans le réseau sentinelle HTC")
```

```{r,echo=F, fig.show = "hold" ,out.width = '50%', out.extra='',comment='', message=FALSE , warning=FALSE}
#source("import1.r");source("preprocessing.R");require(ggplot2); require(gridExtra)
#generate plot per site:
#source("reporting_plot.R",local = T)
