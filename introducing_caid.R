...
if ( input$Cluster_algo=="Total")
{
  ...
  Nbsites_received=caid[,sum(caid_value,na.rm = T),by="code"]
  setnames(Nbsites_received,"V1","numerateur")
  Nbsites_actif=caid[is.na(caid_value)==F,length(unique(sites)),by="code"]
  setnames(Nbsites_actif,"V1","denominateur")
  caid= merge(Nbsites_received,Nbsites_actif,by.x="code",by.y="code",all.x=T)
  caid[,mild_value:=numerateur/denominateur]
  ...
  Nbsites_received=caid[,sum(caid_value,na.rm = T),by="code,facies"]
  setnames(Nbsites_received,"V1","numerateur")
  Nbsites_actif=caid[is.na(caid_value)==F,length(unique(sites)),by="code,facies"]
  setnames(Nbsites_actif,"V1","denominateur")
  caid= merge(Nbsites_received,Nbsites_actif,
              by.x=c("code","facies"),
              by.y=c("code","facies"),all.x=T)
  caid[,caid_value:=numerateur/denominateur]
  cat('DONE\n')
}